// test_vl_codebook_decoder.cpp — Verilator co-simulation tests for codebook_decoder
// Mirrors test/systemc/test_codebook_decoder.cpp against the Kanagawa-generated RTL.

#include "kanagawa_harness.h"
#include "Vcodebook_decoder.h"

#include <cassert>
#include <cstdint>
#include <cstdio>

using Harness = KanagawaHarness<Vcodebook_decoder>;

// IQ3S grid — copied from model/systemc/src/codebook_decoder.h
...
// Populate the codebook grid via write_entry.
static void populate_grid(Harness& h) {
  auto* d = h.dut();
  for (int i = 0; i < 512; ++i) {
    h.wait_ready(d->write_entry_rdy_out);
    d->write_entry_valid_in = 1;
    d->write_entry_index_in = i & 0x1FF;
    d->write_entry_value_in = iq3s_grid[i];
    h.tick();
    d->write_entry_valid_in = 0;
    h.drain_fifo(d->write_entry_rden_in, d->write_entry_empty_out);
  }
}

// Decode a single index on all 4 channels and return magnitudes.
struct Mag4 { uint8_t m0, m1, m2, m3; };

static Mag4 call_decode(Harness& h, uint16_t index) {
  auto* d = h.dut();
  // Fire all 4 decode channels with the same index
  h.wait_ready(d->decode_m0_rdy_out);
  d->decode_m0_valid_in = 1;
  d->decode_m0_index_in = index & 0x1FF;
  d->decode_m1_valid_in = 1;
  d->decode_m1_index_in = index & 0x1FF;
  d->decode_m2_valid_in = 1;
  d->decode_m2_index_in = index & 0x1FF;
  d->decode_m3_valid_in = 1;
  d->decode_m3_index_in = index & 0x1FF;
  h.tick();
  d->decode_m0_valid_in = 0;
  d->decode_m1_valid_in = 0;
  d->decode_m2_valid_in = 0;
  d->decode_m3_valid_in = 0;

  Mag4 r;
  r.m0 = h.read_fifo(d->decode_m0_rden_in, d->decode_m0_empty_out, d->decode_m0_result_out);
  r.m1 = h.read_fifo(d->decode_m1_rden_in, d->decode_m1_empty_out, d->decode_m1_result_out);
  r.m2 = h.read_fifo(d->decode_m2_rden_in, d->decode_m2_empty_out, d->decode_m2_result_out);
  r.m3 = h.read_fifo(d->decode_m3_rden_in, d->decode_m3_empty_out, d->decode_m3_result_out);
  return r;
}

// Verify read_entry round-trips a written value.
static uint32_t call_read_entry(Harness& h, uint16_t index) {
  auto* d = h.dut();
  h.wait_ready(d->read_entry_rdy_out);
  d->read_entry_valid_in = 1;
  d->read_entry_index_in = index & 0x1FF;
  h.tick();
  d->read_entry_valid_in = 0;
  return h.read_fifo(d->read_entry_rden_in, d->read_entry_empty_out, d->read_entry_result_out);
}
...
static void test_first_entry() {
  ...
  h.reset();
  populate_grid(h);
  Mag4 r = call_decode(h, 0);
  // grid[0] = 0x01010101 -> (1,1,1,1)
  assert(r.m0 == 1);
  assert(r.m1 == 1);
  assert(r.m2 == 1);
  assert(r.m3 == 1);
...
static void test_second_entry() {
  ...
  h.reset();
  populate_grid(h);
  Mag4 r = call_decode(h, 1);
  // grid[1] = 0x01010103 -> m0=3, m1=1, m2=1, m3=1
  assert(r.m0 == 3);
  assert(r.m1 == 1);
  assert(r.m2 == 1);
  assert(r.m3 == 1);
...
static void test_all_entries_valid_magnitudes() {
  ...
  populate_grid(h);
  for (int i = 0; i < 512; ++i) {
    Mag4 r = call_decode(h, i);
    for (int m : {(int)r.m0, (int)r.m1, (int)r.m2, (int)r.m3}) {
      assert(m >= 1 && m <= 15);
      assert(m % 2 == 1);
    }
  }
...
static void test_known_entry_511() {
  ...
  h.reset();
  populate_grid(h);
  Mag4 r = call_decode(h, 511);
  // grid[511] = 0x0f0f0101 -> m0=1, m1=1, m2=15, m3=15
  for (int m : {(int)r.m0, (int)r.m1, (int)r.m2, (int)r.m3}) {
...
int main() {
  ...
  std::puts("\nAll codebook_decoder Verilator co-sim tests passed.");
  return 0;
}
