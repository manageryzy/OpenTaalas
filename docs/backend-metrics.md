...
| embed_rom | 244,742 | 1× nor_rom_65536x192 | 600×36000 | 2,924 | **DRT complete** | -5.86 |
| vector_unit | 790,949 | 2× nor_rom_4096x1024 | 5000×5000 | 54,540 | **DRT complete** | -19.30 |
...
| Design | Std Cells | Macro(s) | Die (µm) | GRT Overflow | DRC | WNS (ns) | fmax (MHz) |
|--------|-----------|----------|----------|-------------|-----|----------|-----------|
| kv_cache_demo | 2,606 | 8× sram_4096x8 | 1200×5000 | **0** | **0** | -0.25 | 235 |
| lm_head_demo | 12,900 | 1× nor_rom_65536x192 | 600×36000 | 2,570 | **0** | -5.40 | 106 |

**kv_cache_demo** — 16 tokens × 8 heads × 128 dims (full scale: 4096 tokens). Proves circular buffer K/V store architecture with 8 SRAM tiles. Timing-clean internally (reg-to-reg slack +1.29ns), WNS is output-port only. 17% utilization.
...
| Academic demos (DRT/GDS) | 2 | ~4.3 mm² | ~27.6 mm² |
| **Routed total** | **19** | **~30.4 mm²** | **~118 mm²** |
...
| DRC violations | 12 (embed_rom), 9 (rope), 782 (vector_unit), 0 (all others) | Stubborn met3 shorts near macro edges |
