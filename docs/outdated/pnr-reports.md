# Place & Route Reports — Macro-Bearing Designs (HISTORICAL — superseded)

> **⚠️ Outdated as of 2026-05-13.** Several per-design metrics here predate the v5 SRAM macro refactor (rmsnorm, swiglu, lut_interp moved to real SRAM macros), the v7 RoPE table/datapath split (rope → rope_gen + rope_apply), and the v12 layer_orchestrator composition. For current per-design metrics, see [`docs/backend-metrics.md`](../backend-metrics.md). Per-design narrative below is kept for historical reference only.
